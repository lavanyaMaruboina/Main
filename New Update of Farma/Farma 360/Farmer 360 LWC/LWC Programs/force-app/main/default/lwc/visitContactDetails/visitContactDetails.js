...
const columns = [
    ...
    { label: 'Land Details', fieldName: 'Land_Details__c' },
...
export default class VisitContactDetails extends NavigationMixin(LightningElement) {
   ...
   /* address = {
    ...
    handleLandSearch(event) {
        ...
        searchLandDetails({ searchKey: this.landName, contactId: this.contactId })
            .then(result => {
                console.log('lands result>>>>>'+JSON.stringify(result));
                // Handle the result
                this.lands = result;
            })
            .catch(error => {
    ...
    handleSave(event) {
        ...
        const fields = {
            ...
            Quantity__c: this.visit.Quantity__c,
            Land_Details__c: this.selectedLandId
        };

        console.log('the adress is Keval1>>>',JSON.stringify(fields));
        console.log('the adress is1>>>',JSON.stringify(fields));
