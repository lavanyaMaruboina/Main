...
export default class Lwc_FinalTermsScreen extends NavigationMixin(LightningElement){
    ...
    if(this.isPVUsedAndRefinance == true){
        let referrerName =await this.referrerNameOptions.find(opt => opt.value === this.referrerNameValue)?.label;
        try{if(referrerName){let splitBenCode = referrerName.split('|'); let benCode = splitBenCode[1];console.log('OUTPUT benCode: ',benCode);this.checkRCLimitEnable(benCode.trim());}}catch(error){console.log('error ref__'+error);}}
    }//CISP-8762
    ...
    if(this.productTypeDetail.toLowerCase() === 'Passenger Vehicles'.toLowerCase()){    if(this.referrerNameValue && this.referrerNameOptions.length>0){
        //let referrerNameVal=this.referrerNameOptions.find(opt => opt.label === this.referrerNameValue)?.value; 
        let referrerNameVal = this.referrerNameOptions.find(opt => opt.value === this.referrerNameValue)?.label;
        if(referrerNameVal){
            try{let splitBenCode = referrerNameVal.split('|'); let benCode = splitBenCode[1];console.log(benCode,'benCode22');this.checkRCLimitEnable(benCode.trim());}catch(error){console.log('error__'+error);}
        }
       }
    }
    ...
    async connectedCallback(){
        ...
        if(this.forTwoWheeler===false){reffnamedata({ loanApplicationId: this.recordid }).then(result => {
            ...
            console.log('this.referrerNameOptions___'+JSON.stringify(this.referrerNameOptions));
        ...
        getbenefitioryDetails({loanApplicationId: this.recordid}).then((res)=>{if(res && res.length>0){console.log(res);if(!(res[0]?.Dealership_Nature__c=='SDR' ||res[0]?.Dealership_Nature__c=='SSP')){this.disabledDealerIncentiveAmountSubDealer=true;this.disableDealerSubPercentage=true;this.isSubDealer=false;}}}).catch(err=>console.error(err));
        //CISP-20504
