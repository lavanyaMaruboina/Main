...
export default class LWC_LOS_getAssetDetails extends NavigationMixin(LightningElement) {
    ...
    @api isRevokedLoanApplication;d2cMMVDisable = false;@track dealerBenCode;@api checkleadaccess;@track showPopupModal = false;@track index;@track showSpinner = true;@track modalMsg;@track vaahanAPIRetryCount = 0;@track dealMasterAPIRetryCount = 0;@track isEnableNext = false;@track isGetVehicleDetailsSuccessful = false;@track isDCategoryVehicle = false;@track disableGetVehicleDetails = false;@track displayUsedVehicleTypeFields = false;@track showVahanDetailsSection = false;@track showSuccessTick = false;@track showFailureTick = false;@track allowSubmit = true;@track vehicleType;@track dealNumber;@track chassisNumber;@track engineNumber;@track lastOwnerName;@track manufacturerYearAndMonth;@track vehicleRegNoValue;@track vehicleRegisteredInNameOf;@track invoiceInNameOf;@track product;@track makeId;@track modelId;@track vehicleDetailId;@track delaytime;@track leadNumber;@track eligibilityLoanAmount;@track tenureInMonths;@track vehicleCategory;@track isDCategoryAsset;@track allFieldsDisabled = false;@track isDisabledSubmit = false;@track lastStage;@track currentStageName;@track agentBLCode;@track make;@track model;@track variant;@track dealerSubDealerName;@track segmentCode;
    @track purposeOfPurchaseOptionsList = [];@track vehicleRegisteredInNameOfOptionsList = [];@track usageTypeOptionsList = [];@track lienInFavorOfOptionsList = [];@track rcRetentionOptionsList = [];@track usageTypeOptionsListTractor = [];@track rcRetentionOptionsListTractor = [];@track numberOfOwnershipsOptionsList = [];@track purposeOfPurchase;@track vehicleRegisteredInNameOf;@track usageType;@track lienInFavorOf;@track rcRetentionApplicable;@track numberOfOwnerships;
    @track makeOptionsList = [];@track modelOptionsList = [];@track variantOptionsList = [];@track dealerSubDealerOptionsList = [];
    @track isEnableUploadViewDoc = true;@track showFileUploadAndView = false;@track insExpiryDate;@track fromHome=false;
    @track leadSource;@track invoiceNameDisabled = false;@track isDealerFieldMandatory = false;@track isDealerFieldDisabled = true;
    aggregatorSource = '';@track isTractor = false;@track assetDetailList = [];@track assetCaseCreated = false;
    labels = {AssetDetailsSaved, RefinanceLabel, EngineNo_Regex, ChassisNo_Regex, lastNameError, RegEx_Alphabets_Only, NameError}
 ...
 && this.product == 'Passenger Vehicles' && !this.isD2cApplication){//CISP-3705
    ...
    async handleSubmit() {
        if(this.isTractor) {
            ...
            await this.validateVehicleCatogory();await this.saveAssetDetails();
        }
    }
    //CISP-4459 start
