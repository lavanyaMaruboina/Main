...
async handleAepsChange(event) {helper.handleAepsChange(this, event);}
    ...
    async goToCaptureDedupe(){if(this.currentStageName === 'Loan Initiation' && this.lastStage === 'Loan Initiation' && !this.isNonIndividualBorrower) {console.log('Starting POA Tagging for applicantId:', this.applicantId);await poaTagging({ applicantId: this.applicantId });console.log('POA Tagging completed for applicantId:', this.applicantId);this.callCibilTextMatchAPI();}this.currentStep = this.label.captureDedupe;this.isStepFour = false;this.isStepFive = true;const applicantsFields = {};applicantsFields[APP_ID_FIELD.fieldApiName] = this.applicantId;applicantsFields[APPLICANT_SUBSTAGE.fieldApiName] = this.label.captureDedupe;this.updateRecordDetails(applicantsFields);}
