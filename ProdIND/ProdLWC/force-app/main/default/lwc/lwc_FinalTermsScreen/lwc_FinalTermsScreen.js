...
export default class Lwc_FinalTermsScreen extends NavigationMixin(LightningElement){
    ...
    if(this.productTypeDetail.toLowerCase() === 'Passenger Vehicles'.toLowerCase()){    if(this.referrerNameValue && this.referrerNameOptions.length>0){
        let referrerNameVal=this.referrerNameOptions.find(opt => opt.value === this.referrerNameValue).label; if(referrerNameVal){let splitBenCode = referrerNameVal.split('|'); let benCode = splitBenCode[1];console.log(benCode,'benCode22');this.checkRCLimitEnable(benCode.trim())};
       }
    }
