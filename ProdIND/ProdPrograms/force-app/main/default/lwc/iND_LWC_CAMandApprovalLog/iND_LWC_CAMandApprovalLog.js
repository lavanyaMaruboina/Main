...
export default class IND_LWC_CAMandApprovalLog extends NavigationMixin(LightningElement) {
   ...
   // fundedIns;//CISP-7754 
    ...
    forwardCAM() {
        ...
        if (((this.forwardTo == 'SCM' || this.forwardTo == 'NCM' ) || this.hasSentToSCM == true) || this.hasCommitteeDeviation == false) {
            if (((this.forwardTo == 'CA' || this.hasSentToCA == true) || ((this.forwardTo == 'SCM' || this.forwardTo == 'NCM') && !this.hasSentToSCM)) || this.hasCommitteeDeviation == false) {
                this.showSpinner = true;
                this.remarks = this.remarks;
                let method = this.hasCommitteeDeviation == false ? getForwardWrapper : forwardCAMWithAPI ;
            
                method({ camId: this.camRecId, role: this.forwardTo })
                    .then(response => {
                        if (response) {
                                    ...
                                    if (result && result.response && result.response.status == 'SUCCESS') {
                                        ...
                                        createProposalLog_MTD({ camId: this.camRecId, role: this.forwardTo, remarks: this.remarks, emailTo: emailRequestWrapper.emailTo })
                                            .then(element => {
                                                if (element == true) {
                                                    count += 1;
                                                    if (this.forwardTo == 'SCM' || this.forwardTo == 'NCM') {
